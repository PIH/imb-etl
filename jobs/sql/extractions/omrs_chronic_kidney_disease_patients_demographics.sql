CREATE temporary table temp_chronic_kidney_disease_patients_demographics (
...
# Populate the "rows" of this table to contain all chronic_kidney_disease patients
insert into temp_chronic_kidney_disease_patients_demographics(Patient_UUID,patient_id,Medical_Record_Number,Patient_Names,
    Age,Gender,country,state_province,district,sector,cell,umudugudu,health_facility,
    phone_number,religion,profession,
    Patient_status,Treatment_status)
...
and pro.name = 'Chronic Kidney Disease'
...
WHERE  pr.name = 'Chronic Kidney Disease'
...
from patient_program pp
         inner join temp_chronic_kidney_disease_patients_demographics t on pp.patient_id = t.patient_id
where pp.voided = 0 and pp.program_id= (select program_id from program where name="Chronic Kidney Disease")
;
...
update temp_chronic_kidney_disease_patients_demographics tdpd
 set
    tdpd.Enrolment_Date =
    (
        SELECT
        ...
        where
            ...
            and ppro.program_id=(select program_id from program where name="Chronic Kidney Disease")
...
select * from temp_chronic_kidney_disease_patients_demographics;