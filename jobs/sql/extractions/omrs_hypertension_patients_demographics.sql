CREATE temporary table temp_hypertension_patients_demographics (
...
# Populate the "rows" of this table to contain all hypertension patients 
insert into temp_hypertension_patients_demographics(Patient_UUID,patient_id,Medical_Record_Number,Patient_Names,
    Age,Gender,country,state_province,district,sector,cell,umudugudu,health_facility,
    phone_number,religion,profession,
    Patient_status,Treatment_status,Evaluation_status)
...
and pro.name = 'Hypertension program'
...
WHERE  pr.name = 'Hypertension program'
...
from patient_program pp
         inner join temp_hypertension_patients_demographics t on pp.patient_id = t.patient_id
where pp.voided = 0 and pp.program_id= (select program_id from program where name="Hypertension program")
;
...
update temp_hypertension_patients_demographics tdpd
 set 
    tdpd.Enrolment_Date =
    (
        SELECT 
        ...
        where
            ...
            and ppro.program_id=(select program_id from program where name="Hypertension program")
...
select * from temp_hypertension_patients_demographics;