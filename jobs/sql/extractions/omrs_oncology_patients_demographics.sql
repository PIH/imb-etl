CREATE temporary table temp_oncology_patients_demographics (
...
# Populate the "rows" of this table to contain all oncology patients
insert into temp_oncology_patients_demographics(Patient_UUID,patient_id,Medical_Record_Number,Patient_Names,
...
where prog.name = 'Oncology Program'
and ppro.voided=0
group by patient_id
...
WHERE  pr.name = 'Oncology Program'
and pp.voided = 0
and pr.retired = 0
...
from patient_program pp
         inner join temp_oncology_patients_demographics t on pp.patient_id = t.patient_id
where pp.voided = 0 and pp.program_id= (select program_id from program where name="Oncology Program")
;
...
update temp_oncology_patients_demographics tdpd
 set
    tdpd.Enrolment_Date =
    (
        SELECT
        ...
        where
            ...
            and ppro.program_id=(select program_id from program where name="Oncology Program")
...
select * from temp_oncology_patients_demographics;